from keras.layers import Activation, Conv2D, Dropout  # Importation des couches nécessaires de Keras
from keras.layers import AveragePooling2D, BatchNormalization  # Importation des couches nécessaires de Keras
from keras.layers import GlobalAveragePooling2D  # Importation de la couche de global average pooling
from keras.models import Sequential, Model  # Importation des modèles séquentiel et fonctionnel de Keras
from keras.layers import Flatten, Input  # Importation des couches nécessaires de Keras
from keras.layers import MaxPooling2D, SeparableConv2D  # Importation des couches nécessaires de Keras
from keras import layers  # Importation des couches de Keras
from keras.regularizers import l2  # Importation de la régularisation L2
...
def simpler_CNN(input_shape, num_classes):
    ...
    model = Sequential()
    model.add(Conv2D(filters=16, kernel_size=(5, 5), padding='same', name='image_array', input_shape=input_shape))
    model.add(BatchNormalization())
    ...
    model.add(Conv2D(filters=256, kernel_size=(1, 1), padding='same'))
    model.add(BatchNormalization())
    model.add(Conv2D(filters=num_classes, kernel_size=(3, 3), strides=(2, 2), padding='same'))
...
def tiny_XCEPTION(input_shape, num_classes, l2_regularization=0.01):
    """Définition du modèle tiny XCEPTION"""
...
def mini_XCEPTION(input_shape, num_classes, l2_regularization=0.01):
    """Définition du modèle mini XCEPTION"""
    regularization = l2(l2_regularization)
...
def big_XCEPTION(input_shape, num_classes):
    """Définition du modèle big XCEPTION"""
    img_input = Input(input_shape)
